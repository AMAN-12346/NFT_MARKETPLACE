...
export class collectionController {
  ...
  async createCollection(req, res, next) {
    ...
    try {
      ...
      let contractAddressCheck = await findCollection({
        contractAddress: contractAddress,
      });
      if (contractAddressCheck)
        throw apiError.notFound(responseMessage.CONTRACT_ALREADY_EXIST);
      if (validatedBody.brandId) {
        var brandRes = await findBrand({
          _id: validatedBody.brandId,
          brandApproval: "APPROVED",
        });
        if (!brandRes)
          throw apiError.notFound(
            responseMessage.DATA_NOT_FOUND_APPROVAL_PENDING
          );
      ...
      if (req.files) {
        for (let i = 0; i < req.files.length; i++) {
          if (req.files[i].fieldname=="collectionImage") {
            collectionImage = await commonFunction.getImageUrl1(req.files[i]);
          }
          if (req.files[i].fieldname=="bannerImage") {
            bannerImage = await commonFunction.getImageUrl1(req.files[i]);
          }
        }
      }
      let obj = {
      ...
      return res.json(
        new response(collectionResult, responseMessage.COLLECTION_ADDED)
      );
    } catch (error) {
  ...
  async viewCollection(req, res, next) {
    ...
    try {
      ...
      var bundleResult = await findCollection({
        _id: _id,
  ...
  async editCollection(req, res, next) {
    ...
    try {
      ...
      if (validatedBody.collectionImage) {
        validatedBody.collectionImage = await commonFunction.getSecureUrl(
          validatedBody.collectionImage
        );
      }
      var bundleResult = await findCollection({
      ...
      if (validatedBody.isPromoted === true) {
        ...
        validatedBody["tillDate"] = new Date(
          new Date().setDate(new Date().getDate() + 30)
        ).toISOString();
      }
      var result = await updateCollection(
        { _id: bundleResult._id },
        validatedBody
      );
  ...
  async myCollectionList(req, res, next) {
    ...
    try {
      ...
      let dataResults = await myCollectionPaginateSearch(
        validatedBody,
        userResult._id
      );
