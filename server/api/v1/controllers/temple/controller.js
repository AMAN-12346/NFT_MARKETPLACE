...
import { templeServices } from '../../services/temple';
const {findtempleContent,updatetempleContent,templeList} = templeServices;
...
export class tampleController {
     ...
     *     tags:
     *       - TEMPLE
     *     description: viewTample ?? Get perticular tample details 
     *     produces:
     *       - application/json
     *     parameters:
     *       - name: token
    ...
    async viewTemple(req, res, next) {
        try {
            ...
            var result = await findtempleContent({ templeNumber: validatedBody.templeNumber })
     ...
     *     tags:
     *       - TEMPLE
     *     description: editTemple ?? Edit static content details of termsConditions, privacyPolicy, aboutUs & contactUs.
     *     produces:
     *       - application/json
     *     parameters:
     *       - name: token
     ...
     *         required: true
     *       - name: editTemple
     *         description: editTemple
     *         in: body
     *         required: true
     *         schema:
     *           $ref: '#/definitions/editTemple'
     *     responses:
     *       200:
     *         description: Returns success message
     */

    async editTemple(req, res, next) {
        const validationSchema = {
            _id: Joi.string().required(),
            templeName : Joi.string().optional(),
            templeLength: Joi.string().optional()
        };
        try {
            ...
            var result = await updatetempleContent({ _id: validatedBody._id }, validatedBody)
    ...
    async templeList(req, res, next) {
        try {
            var result = await templeList()
            return res.json(new response(result, responseMessage.DATA_FOUND));
        } catch (error) {
            return next(error);
        }
...
export default new tampleController()